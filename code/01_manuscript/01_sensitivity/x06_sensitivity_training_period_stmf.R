...
bsn2 <- 
  ...
  mutate(bsn_lp = ifelse(bsn_lp < 0, 0, bsn_lp),
         ...
         exc = case_when(out == 1 & psc > 1 ~ "Excess",
                         out == 1 & psc < 1  ~ "Deficit",
                         out == 0 ~ "No-change",
                         TRUE ~ "oth"), 
         exc = factor(exc, 
...
ggsave("figures/01_manuscript/figS21_training_periods_distributions.png",
       w = 8,
       h = 8)  
...
ggsave("figures/01_manuscript/figS22_training_periods_proportions.png",
       w = 8,
       h = 8)  
...
tot_stats %>% 
  ...
  coord_cartesian(ylim = c(0.9, 1.3), xlim = c(2019.8, 2021.2))+
  scale_color_manual(values = cols)+
  labs(title = "Population-weighted median p-scores",
       y = "P-score")+
  theme_bw()+
