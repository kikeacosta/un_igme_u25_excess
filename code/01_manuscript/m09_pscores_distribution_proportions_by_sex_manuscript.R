...
pop_income <- 
  ...
  # filter(Sex == "t") %>% 
  select(income = Income, year = Year, sex = Sex, age = Age, pop_tot) %>% 
...
dts2 <- 
  ...
  mutate(sex = case_when(sex == "f" ~ "Females",
                         sex == "m" ~ "Males"),
         exc = case_when(up > 1 & lp > 1 ~ "Excess",
                         up < 1 & lp < 1 ~ "Deficit",
                         TRUE ~ "No-change"),
         exc = factor(exc, 
...
tot_stats <- 
  wgths %>% 
  group_by(sex, age, age_type, year) %>% 
...
w_av1 <- 
  tot_stats %>% 
  select(sex, age, year, mean)

inc_stats <- 
  wgths %>% 
  group_by(sex, age, age_type, income, year) %>% 
  do(compute_qs(chunk = .data)) %>% 
  ungroup()
...
pl_dist <-
  dts2 %>%
    ...
    facet_nested(~ year+sex, scales = "free", space = "free_y",
  ...
  db_summary <- 
    dts2 %>% 
    select(country, year, sex, age, age_type, psc, lp, up, exc_all = exc) %>% 
    ...
    mutate(exc_out = case_when(psc_out > 1 ~ "Excess",
                               psc_out < 1 ~ "Deficit",
                               psc_out == 1 ~ "No-change")) %>% 
    select(year, sex, age, age_type, country, exc_all, exc_out) %>% 
    group_by(year, sex, age) %>% 
    mutate(N_cts = n()) %>% 
    ungroup() %>%  
    gather(exc_all, exc_out, key = type, value = is_excess) %>% 
    group_by(year, sex, age, age_type, N_cts, type, is_excess) %>% 
    summarise(N = n()) %>% 
    ungroup() %>% 
    mutate(prop = N / N_cts,
...
ggsave("figures/01_manuscript/figS18_sex_diffs.png",
       dpi = 700,
       width = 8, height = 5)
