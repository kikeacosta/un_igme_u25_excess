...
for(y in 2015:2020){
  bts_temp <- 
    read_csv(paste0("Data/Mexico/Births/mx_annual_births_state_", y, ".csv"),
              skip = 9) %>% 
    rename(state_cod = 1,
           state = 2) %>% 
    drop_na(state) %>% 
...
for(y in 2015:2020){
  sbs_temp <- 
    read_csv(paste0("Data/Mexico/fetal_deaths/state_gestation_", y, ".csv"),
              skip = 9) %>% 
    drop_na(Total) %>% 
    rename(weeks = 1) %>% 
...
for(y in 2015:2020){
  inf_temp <- 
    read_csv(paste0("Data/Mexico/infant_deaths/mx_inf_deaths_state_", y, ".csv"),
             skip = 11) %>% 
    rename(measure = 1) %>% 
    drop_na(2) %>% 
