...
bts_st <- 
  read_tsv("Data/USA/births/wonder_births_state.txt") %>% 
  drop_na(Year) %>% 
  select(state = 2,
         year = 4,
         bts = 6)
...
# ~~~~~~~~~~~
sbs <- 
  read_tsv("Data/USA/fetal_deaths/fetal_deaths_us_weeks_2015-2019.txt") %>% 
  drop_na(Year) %>% 
  select(year = 2,
         weeks = 4,
         value = 6) %>% 
...
sbs_20 <- 
  read_fwf("Data/USA/fetal_deaths/Fetal2020US_COD.txt" , 
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
sbs_st <- 
  read_tsv("Data/USA/fetal_deaths/fetal_deaths_state_weeks_2015-2019.txt") %>% 
  drop_na(Year) %>% 
  select(year = 6,
         state = 2,
         weeks = 4,
...
sbs_st_20 <- 
  read_csv("Data/USA/fetal_deaths/us_fetal_deaths_state_2018_2020.csv") %>% 
...
# neonatal
neo_st <- 
  read_tsv("Data/USA/wonder_neonatal_state.txt") %>% 
  drop_na(Year) %>% 
  select(year = 4,
         state = 2,
         value = 6) %>% 
