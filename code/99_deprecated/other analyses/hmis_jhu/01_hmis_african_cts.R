...
file_name <- "Continuity of Essential Services - Mortality-longformat 2022-07-22.xlsx"
...
db_in <- 
  read_xlsx(paste0("Data/HMIS/", file_name),
            sheet = 1) 
...
db_all <-
  ...
  mutate(Month = match(Month, month.abb),
         ...
         indic = recode(indic,
                        !!!indics)) %>% 
  select(-'Indicator (Long)', -Section, -Completeness, -ISO, -Definition)
...
stillb <- 
  ...
  select(Country, Date, Year, Quarter, Month, sbs = Value)

births <- 
  db_all %>% 
  filter(indic == 'deliveries') %>% 
  select(Country, Date, Year, Quarter, Month, dvs = Value) %>% 
  left_join(stillb) %>% 
  mutate(bts = dvs - sbs) %>% 
  select(-dvs, -sbs)
...
neo <- 
  ...
  select(Country, Date, Year, Quarter, Month, neo = Value)

# stillbirth rates
stbs_rate <- 
  births %>% 
  left_join(stillb) %>% 
  mutate(Value = 1000 * sbs / (sbs + bts),
         indic = "stb_rates") %>% 
...
db_all3 %>% 
  filter(indic %in% c("neonatal_rates", 
                      "stb_rates", 
                      "peri_neonatal_rates")) %>% 
  mutate(indic = factor(indic, levels = c("stb_rates", 
                                          "peri_neonatal_rates",
                                          "neonatal_rates"))) %>% 
  ggplot()+
  geom_line(aes(Date, Value))+
