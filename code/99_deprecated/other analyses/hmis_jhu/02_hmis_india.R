...
db <- read_csv(here("Data", "HMIS", "HMIS_data_India_byState_2015_2021.csv"))
...
db2 <- 
  ...
  mutate(Indicator = recode(Indicator,
                            ...
                            "Facility deliveries" = "facility_deliveries",
...
db2 %>% 
  filter(Region == "All India",
         Indicator %in% c("home_deliveries", "facility_deliveries")) %>% 
  ggplot()+
  geom_line(aes(date, value))+
  facet_grid(~ Indicator)

db2 %>% 
  filter(Indicator %in% c("home_deliveries", "facility_deliveries")) %>% 
  ggplot()+
  geom_line(aes(date, value))+
...
db_rates <- 
  ...
  mutate(neonatal_rate = neonatal / live_births,
         still_rate = stillbirths / (facility_deliveries + facility_deliveries),
         # all deliveries are much higher than stillbirths + live_births!!
         test = stillbirths + live_births,
         deliveries_all = facility_deliveries + facility_deliveries,
...
db_rates %>% 
  filter(Region == "All India") %>% 
  ggplot()+
  geom_line(aes(Year, test))+
  geom_line(aes(Year, deliveries_all), col = "red")
...
db_rates %>%
  filter(Region == "All India") %>% 
  ggplot()+
  geom_point(aes(Year, neonatal_rate))+
  # facet_wrap(~ Region, scales = "free")+
  scale_color_manual(values = c("black", "red"))
...
# maternal mortality
db_annual %>% 
  filter(Indicator == "maternal_deaths") %>% 
  ggplot()+
  geom_point(aes(Year, value))+
  facet_wrap(~ Region, scales = "free")
...
db_still %>% 
  ...
  geom_vline(xintercept = c(ymd("2020-03-24"), ymd("2020-05-31")), 
             col = "blue", alpha = 0.5)+
  geom_vline(xintercept = c(ymd("2021-05-15")), 
             col = "red", alpha = 0.5)+
  scale_color_manual(values = c("black", "red"))+
  theme_bw()+
  theme(
...
ggsave("Figures/TAG_pres/India/india_monthly_stillbirths_v2.png", dpi = 600,
       height = 4,
       width = 8)
...
db_still %>% 
  ...
  geom_point(aes(date, still_rate, col = lockdown), size = 0.3)+
  geom_vline(xintercept = c(ymd("2020-03-24"), ymd("2020-05-31")), 
             col = "blue", alpha = 0.5, linetype = "dashed")+
  geom_vline(xintercept = c(ymd("2021-05-15")), 
             col = "red", alpha = 0.5, linetype = "dashed")+
  scale_color_manual(values = c("black", "red"))+
  theme_bw()+
  theme(
    legend.position = "none",
...
ggsave("Figures/TAG_pres/India/all_india_monthly_stillbirths_v2.png", dpi = 600,
       height = 4,
       width = 8)
...
bslns %>% 
  ...
  geom_line(aes(date, baseline), size = 0.5, col = "#023e8a", alpha = 0.7)+
...
ggsave("Figures/HMIS/india_stillbirths_baselines_v3.png", 
       dpi = 1000,
       width = 8,
       height = 4) 
...
bslns %>% 
  ...
  ggplot()+
  geom_ribbon(aes(date, ymin = ll, ymax = ul), alpha = 0.4, fill = "#48cae4")+
  geom_line(aes(date, sbr_bsln), size = 0.5, col = "#023e8a", alpha = 0.7)+
...
ggsave("Figures/HMIS/india_stillbirths_baselines_rates3.png", 
       dpi = 1000,
       width = 8,
       height = 4) 
...
pers <- seq(ymd('2020-03-24'),ymd('2021-05-31'), by = '1 day')
...
ggsave("Figures/TAG_pres/all_india_monthly_neonatal.png", 
       dpi = 600,
       width = 8,
       height = 4)
...
ggsave("Figures/HMIS/india_neonatal_baselines3.png", 
       dpi = 600,
       width = 8,
       height = 4) 
...
ggsave("Figures/HMIS/india_neonatal_baselines_rates3.png", 
       dpi = 1000,
       width = 8,
       height = 4) 
...
ggsave("Figures/HMIS/india_examples_neonatal_baselines_rates2.png", 
       dpi = 1000,
       width = 4,
       height = 4) 
...
p_scores_neo %>% 
  ...
  theme_bw()+
  theme(axis.title.y = element_blank())
ggsave("Figures/HMIS/india_neonatal_pscores2.png", 
       dpi = 600,
       width = 9,
       height = 5) 
