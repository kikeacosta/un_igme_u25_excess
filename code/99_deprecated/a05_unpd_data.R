...
unique(DT$DataSourceShortName)
unique(DT$DataSourceName)

# ==============================================================================
# saving raw unpd data 
# ~~~~~~~~~~~~~~~~~~~~
write_rds(DT, "data_inter/unpd_raw.rds")
...
# rm (list = ls()); gc()
source("Code/00_functions.R")
DT <- read_rds("data_inter/unpd_raw.rds")
# ==============================================================================
...
open_ages <- 
  ...
  group_by(Country, Code, Source, src_yr, Year, Sex) %>% 
  filter(Age == max(Age)) %>% 
  ungroup()
...
no_open <- 
  dts %>% 
  anti_join(open_ages %>% 
              select(Country, Source, src_yr, Sex, Year) %>% 
              unique()) %>% 
  select(Country, Source, src_yr, Year, Sex) %>% 
  unique()
...
dts2 <- 
  ...
  left_join(open_ages %>% 
              select(Country, Source, src_yr, Year, Sex, max_age = Age)) %>% 
  filter(Age < max_age | is.na(max_age)) %>% 
  # adding open age intervals 
  bind_rows(open_ages) %>% 
  arrange(Country, Source, src_yr, Year, Sex, Age) %>% 
  select(-max_age) %>% 
  anti_join(no_open)

# test for duplicates 
dts2 %>% 
  group_by(Country, Code, Source, src_yr, Year, Sex, Age, AgeSpan) %>% 
  mutate(n = n()) %>% 
  ungroup() %>% 
  filter(n > 1)
...
child_ages <-
  ...
  group_by(Country, Code, Source, src_yr, Year, Sex, Age, AgeLabel, AgeSpan) %>% 
...
unnecess_0_4 <- 
  ...
  filter(AgeLabel == "0-4") %>% 
  select(Country, Source, src_yr, Year, Sex, Age, AgeLabel) %>% 
  unique()
...
inc4 <- 
  dts4 %>% 
  select(Country, Code, Source, src_yr, Year, Sex) %>% 
  unique()
...
age_unk_in <- 
  dts %>% 
  inner_join(dts3 %>% select(Country, Source, src_yr, Sex, Year) %>% unique()) %>%
...
age_tot_no <- 
  dts4 %>% 
  anti_join(age_tot_in %>% select(Country, Source, src_yr, Sex, Year) %>% unique()) %>% 
  bind_rows(age_unk_in) %>% 
  group_by(Country, Code, Source, src_yr, Sex, Year) %>% 
  summarise(Deaths = sum(Deaths)) %>% 
  mutate(Age = "TOT",
         AgeSpan = -1) %>% 
  ungroup()
...
tots_all <- 
  ...
  arrange(Country, Code, Source, src_yr, Sex, Year)
...
dts6 <- 
  ...
  group_by(Country, Source, src_yr, Sex, Year) %>% 
...
tot_sex_no <- 
  ...
  anti_join(tot_sex_in %>% 
              select(Country, Code, Source, src_yr, Year, Age) %>% 
              unique()) %>% 
  # filter(Sex != "t") %>% 
  group_by(Country, Code, Source, src_yr, Year, Age, AgeSpan, AgeLabel) %>% 
  summarise(Deaths = sum(Deaths)) %>% 
  ungroup() %>% 
  mutate(Sex = "t")
...
dts7 <- 
  ...
  arrange(Country, Code, Source, src_yr, Year, Sex, suppressWarnings(as.integer(Age))) 
...
only_t_sex <- 
  ...
  select(Country, Code, Source, src_yr, Year, Sex) %>% 
  unique() %>% 
  group_by(Country, Code, Source, src_yr, Year) %>% 
  filter(n() != 3) %>% 
  select(-Sex)
...
dts8 <-
  dts7 %>% 
  anti_join(only_t_sex) %>% 
  group_by(Country, Source, src_yr, Age, Year) %>%
...
dts9 <- 
  ...
  arrange(Country, Code, Source, src_yr, Year, Sex, Age) 
...
# # series with sufficient years for estimating the baseline
# suff_yrs <- 
#   dts10 %>% 
#   filter(Year < 2020) %>% 
#   group_by(Country, Code, Source, src_yr, Sex, Age, AgeLabel, AgeSpan) %>% 
#   filter(n() >= 3) %>% 
#   ungroup() %>% 
#   select(Country, Code, Source, src_yr, Sex, Age, AgeLabel, AgeSpan) %>% 
#   unique()
# 
# unique(suff_yrs$Country)
# 
# # series with data during the pandemic 2020-2021
# pand_yrs <- 
#   dts10 %>% 
#   filter(Year >= 2020) %>% 
#   group_by(Country, Code, Source, src_yr, Sex, Age, AgeLabel, AgeSpan) %>% 
...
unique(dts_yng$Source)

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 9. standardize sources 
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
dups <- 
  dts_yng2 %>% 
  # select(Country, Code, Source2, src_yr, Year, Sex, Age, age_up, Deaths) %>% 
  group_by(Country, Code, Source, src_yr, Year, Sex, Age, age_up) %>% 
  mutate(n = n()) %>% 
  ungroup() %>% 
  filter(n > 1)
...
dts_yng3 <- 
  ...
  group_by(Country, Code, Source, src_yr, Year, Sex, Age, age_up) %>% 
  filter(Deaths == max(Deaths)) %>% 
  group_by(Country, Code, Source, Year, Sex, Age, age_up) %>% 
  filter(src_yr == max(src_yr)) %>% 
  ungroup() %>% 
  select(-AgeLabel, -src_yr) %>% 
...
unique(dts_yng3$Country)

# saving output
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
write_rds(dts_yng3, "data_inter/unpd.rds")
...
test <- 
  dts_yng3 %>% 
  arrange(Country, Source, Sex, Age, Year)
...
suff_yrs <- 
  dts_yng2 %>% 
  filter(Year < 2020) %>% 
  group_by(Country, Code, Source, Sex, Age, age_up) %>% 
  filter(n() >= 3) %>% 
  ungroup() %>% 
  select(Country, Code, Source, Sex, Age, age_up) %>% 
  unique()
...
pand_yrs <- 
  dts_yng2 %>% 
  filter(Year >= 2020) %>% 
  group_by(Country, Code, Source, src_yr, Sex, Age, age_up) %>% 
  filter(n() >= 1) %>% 
  ungroup() %>% 
  select(Country, Code, Source, src_yr, Sex, Age, age_up) %>% 
  unique()
