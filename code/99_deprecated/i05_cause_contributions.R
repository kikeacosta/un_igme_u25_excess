...
cause_sel <- 
  ...
  ungroup() %>% 
  filter(lag(exc_prp_cum) < 0.9 |
           rank == 1 | 
           (rank == n() & exc_prp_cum < 0.9) | 
           cause %in% c("covid19", "external")) %>% 
  filter(cause %in% c("covid19", "external") | exc_prp >= 0.05) %>% 
...
prop_pos %>% 
  filter(sex == "t") %>% 
  mutate(income = factor(income, levels = c("middle", "high"))) %>% 
  ggplot()+
  geom_bar(aes(fill=cause, y=cause, x=cont),
           position="dodge", stat="identity")+
  facet_nested(income~age, scale = "free", space = "free")+
  geom_vline(xintercept = 0, linetype = "dashed")+
  scale_x_continuous(limits = c(0,1), breaks = bks, labels = lbs)+
  labs(
    # title = "Cause-specific contribution to excess",
    x = "Contribution to excess deaths")+
  theme_bw()+
  theme(legend.position = "none",
...
ggsave("figures/causes2/causes_cont_excess_income_all.png",
       w = 6, h = 3)

# selecting causes
# ~~~~~~~~~~~~~~~~

css_pos <- c("perinatal", "cancer", "external", "covid19")
...
prop_pos %>% 
  ...
  mutate(cont = ifelse(income == "middle", -cont, cont),
         ...
         cause = factor(cause, levels = css_pos)) %>% 
...
prop_neg <- 
  dt5 %>% 
  filter(exc < 0) %>% 
  filter(!cause %in% c("respiratory", "infectious")) %>% 
  group_by(income, sex, age, cause) %>% 
  summarise(exc = sum(exc)) %>% 
  ungroup() %>% 
  group_by(income, sex, age) %>% 
...
ggsave("figures/causes2/causes_cont_deficit_income_all.png",
       w = 6, h = 3)

# selecting causes for deficit decomposition
css_neg <- c("perinatal", "congenital", "cancer", "external")
...
prop_neg %>% 
  ...
  mutate(cont = ifelse(income == "middle", -cont, cont),
         income = factor(income, levels = c("middle", "high")),
         cause = factor(cause, levels = css_neg)) %>% 
  ggplot(aes(factor(age), cont, fill = income))+
  geom_bar(position="stack", stat="identity")+
  facet_grid(~cause)+
  coord_flip()+
