...
dts2 <- 
  ...
  mutate(exc = case_when(up > 1 & lp > 1 ~ "Positive",
         ...
         age_type = case_when(age %in% c("Stillbirths", "Neonatal") ~ "Perinatal", 
                              age %in% c("Infant", "1-4") ~ "Child", 
                              TRUE ~ "Youth"),
         age_type = factor(age_type, levels = rev(c("Perinatal", "Child", "Youth"))))
...
plot_grid(pl_dist, NULL, pl_prop, 
          # labels = c("A", "B"),
          # label_size = 14,
          rel_widths = c(1.3, 0.05, 1),
          ncol = 3)

ggsave("Figures/slides/ced_flash_session.png", 
       dpi = 700,
       width = 11, height = 4)
