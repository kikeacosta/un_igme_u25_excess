...
unique(db_all_fit$Age)
unique(db_all_fit$upper_age)
...
db_gr1 <- 
  ...
  mutate(exc = case_when(up > 1 & lp > 1 ~ "Positive",
         ...
         Age = factor(Age, levels = c("0", "5", "10", "15", "20")),
...
# ggsave(paste0("Figures/TAG_pres/excess_young_ages_sex_group1.png"), dpi = 600,
#        width = 10, height = 5)
...
db_gr2 <- 
  ...
  mutate(exc = case_when(up > 1 & lp > 1 ~ "Positive",
         ...
         out = ifelse(exc == "No-excess", 0.5, 1),
         ins = ifelse(exc == "No-excess", 0.6, 0.4),
         Age = factor(Age, levels = c("0_4", "5_9", "10_14", "15_24")),
...
# ggsave(paste0("Figures/TAG_pres/excess_young_ages_sex_group2.png"), dpi = 600,
#        width = 10, height = 5)
...
# ggsave(paste0("Figures/TAG_pres/excess_young_ages_sex_group2_no_arm.png"), dpi = 600,
#        width = 10, height = 5)
...
# ggsave(paste0("Figures/TAG_pres/excess_young_ages_sex_group3_no_arm.png"), 
...
db_summary <- 
  ...
  ungroup() %>% 
  mutate(Prop = N / N_cts,
         Age = factor(Age, levels = rev(c("Infant", "1_4", "0_4", "5_9", "10_14", "15_24"))),
         type = recode(type,
                       "exc_all" = "All estimates",
                       "exc_out" = "Inc. uncert."),
         type = factor(type, levels = c("All estimates", "Inc. uncert.")),
         is_excess = factor(is_excess, levels = c("Positive", "No-excess", "Negative")),
         Sex = recode(Sex,
                      "f" = "females",
                      "m" = "males",
                      "t" = "total"))
...
ggsave("Figures/TAG_pres/excess_summary_by_age_sex.png", width = 8, height = 4)
