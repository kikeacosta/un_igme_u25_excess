...
dts <- 
  ...
  group_by(Country, Source, src_yr) %>% 
...
# test <- 
#   dts %>% 
#   group_by(Country, Code, Source, Year, Sex, Age, AgeLabel) %>% 
#   # most recent update of the same source
#   filter(src_yr == max(src_yr))
...
age_unk_in <- 
  dts %>% 
  inner_join(dts3 %>% select(Country, Source, Sex, Year) %>% unique()) %>%
...
#   filter(n() >= 3) %>% 
#   ungroup() %>% 
#   select(Country, Code, Source, Sex, Age, AgeLabel, AgeSpan) %>% 
#   unique()
# 
...
# saving output
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
write_rds(dts_yng3, "data_inter/unpd_test.rds")
