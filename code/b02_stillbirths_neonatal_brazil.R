...
bts <- 
  read_xlsx("data_input/Brazil/births/br_annual_births_state_format.xlsx") 
...
for(y in 2015:2020){
  sbs_temp <- 
    read_xlsx(paste0("data_input/Brazil/fetal_deaths/state_gestation_", y, "_format.xlsx")) %>% 
    rename(state_num = 1) %>% 
    drop_na(2) %>% 
...
for(y in 2015:2020){
  inf_temp <- 
    read_delim(paste0("data_input/Brazil/infant_deaths/br_inf_deaths_state_", y, ".csv"),
               delim = ";",
               skip = 3,
