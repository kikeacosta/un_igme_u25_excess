...
db_inf <- 
  db_pe_0 %>% 
  mutate(age = case_when(time_unit %in% c("HORAS", "MINUTOS", "SEGUNDOS") |
                           (time_unit == "DIAS" &  age <= 7) ~ "neo_ear",
                         time_unit == "DIAS" & age >= 8 ~ "neo_lat",
                         time_unit %in% c("MESES") ~ "post_neo"),
         date = make_date(y = AÑO, m = MES, d = 15)) %>% 
  group_by(age, date) %>% 
  summarise(deaths = n()) %>% 
  ungroup() %>% 
  drop_na()

write_rds(db_inf, "Output/infant/peru_monthly_inf_deaths_2017_2021.rds")
